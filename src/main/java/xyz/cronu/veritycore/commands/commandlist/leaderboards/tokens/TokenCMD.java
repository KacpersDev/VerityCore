package xyz.cronu.veritycore.commands.commandlist.leaderboards.tokens;
...
public class TokenCMD extends CommandBase {
	@Override
	...
	public List<SubcommandBase> getSubcommands() {
		return Arrays.asList(new TokenTopCMD());
	}
	...
	public void perform(CommandSender sender, Command command, String label, String[] args) throws IOException, DataException {
		if(sender instanceof Player){
			...
			if(args.length == 0){
				...
				Text.message(player, Prefix.SERVER.getPrefix() + "&7You currently have &c" + Number.pretty(prisonPlayer.getTokens()) + " &7Tokens!");
			}
		} else {
