...
public class AddSubCMD extends SubcommandBase {
	@Override
	...
	public String getSyntax() {
		return "/admin add <type> <player> <amount>";
	}
		...
		if (sender.isOp() || sender.hasPermission("admin.add")) {
			if (args.length == 4) {
				...
				if(target == null) { // Checks if the player entered is online or invalid.
					...
					switch (type) {
						case "TOKENS":
							offlinePrisonPlayer.addTokens(amount);
							break;
						case "MONEY":
							offlinePrisonPlayer.addMoney(amount);
							break;
						case "BLOCKS":
							offlinePrisonPlayer.addBlocksMined((long) amount);
							break;
						case "PRESTIGE":
							offlinePrisonPlayer.addPrestige((long) amount);
							break;
					}
					...
					switch (type) {
						case "TOKENS":
							prisonPlayer.addTokens(amount);
							break;
						case "MONEY":
							prisonPlayer.addMoney(amount);
							break;
						case "BLOCKS":
							prisonPlayer.addBlocksMined((long) amount);
							break;
						case "PRESTIGE":
							prisonPlayer.addPrestige((long) amount);
							break;
					}
