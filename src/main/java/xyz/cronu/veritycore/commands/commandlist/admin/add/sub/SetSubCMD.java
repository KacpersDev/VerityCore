...
public class SetSubCMD extends SubcommandBase {
	@Override
		...
		if (sender.isOp() || sender.hasPermission("admin.set")) {
			if (args.length == 4) {
				...
				if(target == null) { // Checks if the player entered is online or invalid.
					...
					switch (type) {
						case "TOKENS":
							offlinePrisonPlayer.setTokens(amount);
							break;
						case "MONEY":
							offlinePrisonPlayer.setMoney(amount);
							break;
						case "BLOCKS":
							offlinePrisonPlayer.setBlocksMined((long) amount);
							break;
						case "PRESTIGE":
							offlinePrisonPlayer.setPrestige((long) amount);
							break;
					}
					...
					switch (type) {
						case "TOKENS":
							prisonPlayer.setTokens(amount);
							break;
						case "MONEY":
							prisonPlayer.setMoney(amount);
							break;
						case "BLOCKS":
							prisonPlayer.setBlocksMined((long) amount);
							break;
						case "PRESTIGE":
							prisonPlayer.setPrestige((long) amount);
							break;
					}
