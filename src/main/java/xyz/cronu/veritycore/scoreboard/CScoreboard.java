package xyz.cronu.veritycore.scoreboard;
...
public class CScoreboard {
	...
	public static void scoreBoard(Player player){
		final CScoreboard scoreboard = new CScoreboard("n", "c", Text.color("&c&lVERITY"));
		PrisonPlayer user = PrisonPlayerManager.getPrisonPlayer(player.getUniqueId());

		CScoreboard.Row row1 = scoreboard.addRow(Text.color("&f&lSTATS"));
		CScoreboard.Row row2 = scoreboard.addRow("");
		CScoreboard.Row row3 = scoreboard.addRow(Text.color("  &cPrestige: &f" + Number.formatted(user.getPrestige())));
		CScoreboard.Row row4 = scoreboard.addRow(Text.color("  &cTokens: &f" + Number.formatted(user.getTokens())));
		CScoreboard.Row row5 = scoreboard.addRow(Text.color("  &cMoney: &f" + Number.formatted(user.getMoney())));
		CScoreboard.Row row6 = scoreboard.addRow(Text.color("  &cBlocks Mined: &f" + Number.formatted(user.getBlocksMined())));
		CScoreboard.Row row7 = scoreboard.addRow("");
		CScoreboard.Row row8 = scoreboard.addRow(Text.color("&f&lSERVER"));
		CScoreboard.Row row9 = scoreboard.addRow("");
		CScoreboard.Row row10 = scoreboard.addRow(Text.color("  &cVote Party:"));
		scoreboard.finish();
		scoreboard.display(player);

		Bukkit.getScheduler().scheduleAsyncRepeatingTask(VerityCore.getPlugin(), new Runnable() {
			@Override
			public void run() {
				row1.setMessage(Text.color("&f&lSTATS"));
				row2.setMessage("");
				row3.setMessage(Text.color("  &cPrestige: &f" + Number.formatted(user.getPrestige())));
				row4.setMessage(Text.color("  &cTokens: &f" + Number.formatted(user.getTokens())));
				row5.setMessage(Text.color("  &cMoney: &f" + Number.formatted(user.getMoney())));
				row6.setMessage(Text.color("  &cBlocks Mined: &f" + Number.formatted(user.getBlocksMined())));
				row7.setMessage("");
				row8.setMessage(Text.color("&f&lSERVER"));
				row9.setMessage("");
				row10.setMessage(Text.color("  &cVote Party:"));
			}
		}, 20, 80);
	}
		...
		public class Row {
			...
			private @Nullable ChatColor charsToChatColor(char[] chars){
